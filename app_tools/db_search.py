# azure_image_search.py
import os, json, base64, http.client, urllib.parse, requests
...
SEARCH_ADMIN_KEY = os.getenv("AZURE_SEARCH_ADMIN_KEY", "").strip()
...
DEFAULT_IMAGES_DIR = Path(os.getenv("IMAGES_DIR", r"D:\serp\imagesearch\images"))
DEFAULT_OUTPUT_JSON= Path(os.getenv("OUTPUT_JSON", r"output\docVectors.json"))
...
@retry( #!
    reraise=True,
    retry=retry_if_exception_type((requests.exceptions.Timeout, http.client.HTTPException)),
...
def search_similar(query_image_path: str, k: int = 3, index_name: str = INDEX_NAME, images_dir: Path = DEFAULT_IMAGES_DIR):
    cred = AzureKeyCredential(SEARCH_ADMIN_KEY)
    sc = SearchClient(endpoint=SERVICE_ENDPOINT, index_name=index_name, credential=cred)

    vec = get_image_vector(query_image_path, AI_VISION_KEY, _normalize_region(AI_VISION_REGION), AI_VISION_ENDPOINT)
    ...
    for r in sc.search(search_text=None, vector_queries=[vq], select=["id", "description"]):
        # decode Base64 key back to path
        try:
            decoded = base64.urlsafe_b64decode(r["id"]).decode("utf-8")
        except Exception:
            decoded = r["id"]  # fallback
